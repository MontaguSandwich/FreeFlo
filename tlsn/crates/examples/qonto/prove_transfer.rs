...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let api_key_login = env::var("QONTO_API_KEY_LOGIN")
        .expect("QONTO_API_KEY_LOGIN environment variable required");
...
async fn prover<S: AsyncWrite + AsyncRead + Send + Sync + Unpin + 'static>(
        ...
        if let Ok(json) = serde_json::from_str::<serde_json::Value>(&body_str) {
            if let Some(transactions) = json.get("transactions").and_then(|t| t.as_array()) {
                if let Some(tx) = transactions.first() {
                    println!("\n📋 Transaction Found:");
                    println!("   ID: {}", tx.get("id").and_then(|v| v.as_str()).unwrap_or("N/A"));
                    println!("   Amount: €{:.2}", tx.get("amount").and_then(|v| v.as_f64()).unwrap_or(0.0));
                    println!("   Amount (cents): {}", tx.get("amount_cents").and_then(|v| v.as_i64()).unwrap_or(0));
                    println!("   Status: {}", tx.get("status").and_then(|v| v.as_str()).unwrap_or("N/A"));
                    println!("   Reference: {}", tx.get("reference").and_then(|v| v.as_str()).unwrap_or("N/A"));
                    if let Some(transfer) = tx.get("transfer") {
                        println!("   Beneficiary IBAN: {}",
                            transfer.get("counterparty_account_number").and_then(|v| v.as_str()).unwrap_or("N/A"));
                    }
                } else {
    ...
    println!("");
    println!("Next steps:");
    println!("  1. Run the presentation step: cargo run --release --example qonto_present_transfer");
