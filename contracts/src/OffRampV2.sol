...
/// @dev Users specify amount + currency, solvers quote with their RTPN routes.
///      User selects best quote, commits funds, solver fulfills.
contract OffRampV2 is ReentrancyGuard, Pausable, Ownable {
    ...
    /// @notice Solver metadata
    mapping(address => SolverInfo) public solverInfo;

    /// @notice Which RTPNs each solver supports
    ...
    function createIntent(
        ...
        // NOTE: USDC is NOT transferred yet - only after quote selection
    ...
    /// @notice Submits a quote for an intent
    ...
    function submitQuote(
        ...
        if (!authorizedSolvers[msg.sender]) revert NotAuthorizedSolver();
        if (!solverSupportsRtpn[msg.sender][rtpn]) revert SolverDoesNotSupportRtpn();
    ...
    /// @notice Fulfills an intent after completing the fiat transfer
    /// @param intentId The intent to fulfill
    /// @param transferId The fiat transfer ID
    /// @param fiatSent Actual fiat amount sent
    function fulfillIntent(
        ...
        if (intent.selectedSolver != msg.sender) revert NotAuthorizedSolver();
        if (block.timestamp > intent.committedAt + FULFILLMENT_WINDOW) revert FulfillmentWindowExpired();
        ...
        emit IntentFulfilled(intentId, msg.sender, transferId, fiatSent);
    }
    ...
    function getQuote(bytes32 intentId, address solver, RTPN rtpn) external view returns (Quote memory) {
    ...
    function getIntentTiming(bytes32 intentId) external view returns (IntentStatus status, uint256 timeRemaining) {
        Intent storage intent = intents[intentId];
    ...
    function registerSolver(address solver, string calldata name) external onlyOwner {
        authorizedSolvers[solver] = true;
        solverInfo[solver] = SolverInfo({
            name: name,
            totalFulfilled: 0,
            totalVolume: 0,
        ...
        emit SolverRegistered(solver, name);
    }

    /// @notice Sets which RTPNs a solver supports
    function setSolverRtpn(address solver, RTPN rtpn, bool supported) external onlyOwner {
        solverSupportsRtpn[solver][rtpn] = supported;
        emit SolverRtpnUpdated(solver, rtpn, supported);
    }

    /// @notice Removes a solver
    function removeSolver(address solver) external onlyOwner {
        authorizedSolvers[solver] = false;
        solverInfo[solver].active = false;
